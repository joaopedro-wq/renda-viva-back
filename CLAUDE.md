...
| `POST /api/importacoes/pre-visualizar`                                    | Protegida — multipart (`arquivo` + `modelo_importacao_id` ou `mapeamento` opcionais). Nunca grava lançamento, no máximo cria um `ModeloImportacao` novo. Cabeçalho Nubank Conta (`Data,Valor,Identificador,Descrição`) é reconhecido automaticamente, sem precisar de `mapeamento`. Lógica movida pra `ImportacaoPreVisualizacaoService`, reaproveitada por `upload-chunk`.     |
| `POST /api/importacoes/upload-chunk`                                      | Protegida — recebe um chunk por vez (`pion/laravel-chunk-upload`, protocolo Blueimp via header `Content-Range: bytes {inicio}-{fim}/{total}`, mesmo nome de arquivo em todo chunk). Chunk intermediário → `{recebido: true}`; chunk final → mesmo formato de `pre-visualizar`. Envio sequencial (sem fila). Naming de chunk por IP+navegador, não por sessão (API é stateless). |
